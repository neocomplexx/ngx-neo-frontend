...
import { UserBasicDTO } from '../../models/DTO/userBasic.DTO';
import { AuthNewUserRequestDTO } from '../../models/DTO/authNewUserRequest.DTO';
import { AuthEditUserRequestDTO } from '../../models/DTO/authEditUserRequest.DTO';
...
export class UsersServiceBackend {
   ...
   public async getUsersAutoComplete(contiene: string, pageSize: number): Promise<Array<UserBasicDTO>> {
      ...
      const res = await this.http.get(this.Constants.apiURL +
          '/users/autoComplete' + '?contiene=' + contiene + '&pageSize=' + pageSize).toPromise();
      const resJson = res['data'];
