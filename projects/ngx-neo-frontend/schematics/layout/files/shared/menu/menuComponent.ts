...
export class MenuComponent {
  ...
  protected armarMenu() {
    ...
    for (const grupo of this.jsonMenuData.grupos) {
      const grupoModel = new MenuGrupoModel(i.toString());
      ...
      for (const opcion of grupo.opciones) {
        ...
        const opcionModel = new MenuOpcionModel();
  ...
  @HostListener('window:keyup', ['$event']) hotkeys(event: KeyboardEvent) {
    ...
    if (event.keyCode >= 65 && event.keyCode <= 90) {
      ...
      let opcionesPosibles: Array<MenuOpcionModel> = new Array<MenuOpcionModel>();
      ...
      if (opcionesPosibles.length > 0) {
        ...
        const primeraOpcion: MenuOpcionModel = this.menus[0].primeraOpcionHabilitada();
